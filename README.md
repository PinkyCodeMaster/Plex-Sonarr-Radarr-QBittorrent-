...
- `docs/deployment.md`: How to run locally (CPU-only or with GPU) or on cloud with cost controls.
...
## Deployment profiles
- **Local CPU-only (default)**: run the stack as-is; leave `/dev/dri` in place or remove it if your host has no iGPU. See `docs/deployment.md`.
- **Local with GPU**: enable Intel Quick Sync (already mapped) or add NVIDIA runtime/env vars if you install `nvidia-container-toolkit`. See `docs/deployment.md`.
- **Cloud (AWS/Azure/GCP)**: start with small CPU instances, attach a single data volume for `/data`, keep qBittorrent on VPN, and only expose 80/443. See `docs/deployment.md` for cost tips.
